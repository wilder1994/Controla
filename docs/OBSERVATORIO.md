...
Un reporte nuevo abre overlay + sonido en empresa (Observatorio), admin del cliente y admin de esa instalación. **Quien reporta no oye ni ve la alerta.** Poll `GET /company/ops/alerts.json` y `/client/ops/alerts.json`. El pánico de portería no cambia. El pánico por usuario (sidebar empresa/cliente y APK) lo oyen usuarios de empresa con Supervisión u Observatorio; quien lo oprime tampoco oye. Quien tiene `ops.panic.attend` puede **Atender** (ficha abierto/cerrado en `/company/panics`). El overlay de pánico suena en bucle hasta Enterado o Atender.
