...
**Parafiscales:** no van en Normoteca. En el listado, **Carga masiva planilla**: Excel PILA (hoja por `CC`+cédula). Overlay con % al leer y al guardar. El recorte clona el encabezado original (estilos, combinadas, logo) y solo las filas de ese colaborador; BJ15 = su total. Ver en modal (tabla) y Descargar xlsx. Mismo periodo reemplaza. No se guarda la planilla completa.
