...
PWA en `field-app/` (copia `public/campo/` y Laragon `Controla_Supervision`). Caché SW `controla-sup-v42`. Hosting: `https://controla.wcodex.cloud/campo`. APK Android (Capacitor) en `field-app/android/`; el binario se publica en `public/downloads/controla-supervision.apk`.
