...
PWA en `field-app/` (copia `public/campo/` y Laragon `Controla_Supervision`). Caché SW `controla-sup-v47`. Hosting: `https://controla.wcodex.cloud/campo`. APK Android v1.6 (Capacitor) en `field-app/android/`; el binario se publica en `public/downloads/controla-supervision.apk`. Selfie de inicio en el círculo de perfil.
...
| Pánico | `POST /panic` | Turno abierto. Sin confirmar. Botón rojo al final, bajo fichas. Avisa a la empresa; este teléfono no suena |
...
El mapa `/company/supervision` usa **satélite** por defecto (toggle Terreno). Pinta **cliente** (índigo) e **instalación** (cian) con GPS; si la sede es “el mismo cliente” y el pin coincide, no se duplica. El mapa de **Mi empresa** usa la misma pareja de pines. En vivo e Historial: **dos columnas** (mapa alto + lista). **En vivo** no lleva filtros en el header: lista todos los turnos abiertos. Por defecto solo motos (sin polilíneas); clic en la fila pinta la ruta GPS de ese supervisor. **Ver** abre la ficha de turno en curso (no se guarda hasta el cierre). Estado En vivo: en ruta / detenido con horas (`Se detuvo a las HH:mm · lleva N min`); paradas cerradas `de HH:mm a HH:mm`. Pin de moto: **En línea**, **Pantalla apagada** o **Sin señal**. Poll 10 s, sin Roads. Pines a ≤50 m se agrupan. La moto queda detrás (`pointer-events: none` en hover: tooltip nombre + señal). Clic en la moto o en el grupo abre la lista de eventos (sin la moto). Fichas de pin: OverlayView compacto (no InfoWindow blanco de Google). Apoyo (cian) y alarma (ámbar) tienen pin. Leyenda en la columna. **Historial:** solo turnos **cerrados**; filtros Desde / Hasta / Supervisor; el mapa espera el clic (una ruta en calle, Snap to Roads; nunca varias a la vez). **Ver** abre la ficha ya guardada. Turno cerrado por el sistema: texto **Cierre por el sistema** y, si el teléfono reportó cola en el GPS, **N registros en cola**. Trail: `BuildSupervisorTrailService` (filtra precisión &gt; 150 m, saltos más rápidos que ~130 km/h y picos aislados; luego ~28 m; parada 75 m y ≥120 s; minutos de detenido actual contra el reloj). El mapa En vivo sigue sin Roads: el filtro evita la polilínea en “sismógrafo”.
...
No es el Historial GPS ni el PPTX. Cada captura (revista, alarma, apoyo, documentos) genera una **ficha inmutable** en HTML carta (`window.print()` / Guardar como PDF). Folio `FC-{año}-{R|A|S|D}{id}` (ej. `FC-2026-R000042`). La **ficha de turno** (`kind=shift`, folio `…-T…`) se arma al abrir turno y **solo se guarda** (`supervisor_shifts.sheet_snapshot`) al cerrar; en En vivo/Historial el botón **Ver**. No se guarda un archivo PDF: se arma al abrirla. Cabecera: logo, nombre y NIT de la **empresa** (Mis datos). Sin marca de la plataforma. La revista copia el **encabezado** al guardar (`sheet_intro`). Sitio: cliente, instalación, puesto.
...
`/company/supervision`: En vivo / Historial / Resumen / **Fichas**. En vivo: sin filtros de header; mapa + tabla (inicio, señal, km, revistas, **Ver**); clic en fila = ruta GPS; `GET /company/supervision/live.json`. Historial: Desde / Hasta / Supervisor; mapa vacío hasta clic; callejero (`GET /company/supervision/turnos/{shift}/ruta`); **Ver** ficha guardada. Resumen y Fichas de campo conservan filtros y **Descargar PPTX**. PPTX: `GET /company/supervision/informe.pptx`. Compositor IA: § Informe PPTX (pendiente).

Servicios: `BuildSupervisionMapService`, `BuildSupervisorShiftSheetService` (ficha de turno; snapshot al cierre), `BuildSupervisorTrailService` (antes de simplificar: descarta `accuracy` > 150 m, saltos más rápidos que ~130 km/h y un ping que el siguiente no confirma; el pin y los km usan esa traza, la señal usa el último ping crudo), `SnapSupervisorTrailToRoadsService`, `ResolveSupervisorShiftDeadlineService` (gabela 30 min), `AutoCloseExpiredSupervisorShiftsService` (comando `supervision:auto-close-shifts`). El cierre manual de la PWA sigue exigiendo fotos (`CloseSupervisorShiftService`).
