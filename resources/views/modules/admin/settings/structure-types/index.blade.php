<x-admin-layout title="Ajustes">
    <div class="space-y-6">
        ...
        <div>
            <p class="text-sm text-slate-400">Catálogo de tipos de estructura para el censo de los clientes. El orden se define en la lista (subir / bajar).</p>
        </div>
        ...
        <div class="grid lg:grid-cols-3 gap-6">
            <div class="lg:col-span-2 rounded-xl border border-slate-800 bg-slate-900 overflow-hidden">
                ...
                <div class="divide-y divide-slate-800">
                        ...
                        <div x-data="{ editing: false }" class="px-4 py-3">
                            <div x-show="!editing" class="flex flex-wrap items-center justify-between gap-3">
                                ...
                                <div class="flex items-center gap-2">
                                    <form method="POST" action="{{ route('admin.settings.structure-types.move-up', $type) }}">
                                        @csrf
                                    ...
                                    <form method="POST" action="{{ route('admin.settings.structure-types.move-down', $type) }}">
                                        @csrf
                            ...
                            <form x-show="editing" x-cloak method="POST" action="{{ route('admin.settings.structure-types.update', $type) }}" class="space-y-3">
                                @csrf
                                @method('PUT')
                                <div>
                    ...
                    @empty
                        <p class="px-4 py-8 text-sm text-slate-500 text-center">Aún no hay tipos. Crea el primero.</p>
                    @endforelse
                </div>
            </div>
