<x-company-layout title="Riesgos">
    @include('modules.company.settings.partials.nav-slots', ['companyNavActive' => 'riesgos'])

    <div class="grid lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 rounded-xl border border-slate-800 bg-slate-900 overflow-hidden">
            <div class="px-4 py-3 border-b border-slate-800">
                <h3 class="text-sm font-semibold text-white">Tipos de riesgo</h3>
                <p class="text-xs text-slate-500 mt-1">Clasificación que usa el supervisor en la recomendación (físico, químico, eléctrico, biológico, público, etc.).</p>
            </div>
                    ...
                    <div x-data="{ editing: false }" class="px-4 py-3">
                        <div x-show="!editing" class="flex flex-wrap items-center justify-between gap-3">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <form method="POST" action="{{ route('company.supervision-risk-types.destroy', $type) }}" onsubmit="return confirm('¿Eliminar este tipo?')">
                                    @csrf
                                    @method('DELETE')
                        ...
                        <form x-show="editing" x-cloak method="POST" action="{{ route('company.supervision-risk-types.update', $type) }}" class="space-y-3">
                            @csrf
                            @method('PUT')
        ...
        <div class="rounded-xl border border-slate-800 bg-slate-900 p-4 h-fit">
            ...
            <form method="POST" action="{{ route('company.supervision-risk-types.store') }}" class="space-y-3">
                @csrf
                <input type="text" name="name" value="{{ old('name') }}" required placeholder="Riesgo físico" class="w-full h-9 rounded-lg bg-slate-950 border border-slate-700 px-3 text-sm text-white">
