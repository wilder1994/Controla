<x-access-layout>
    <div class="-mt-6 -mx-4 sm:-mx-6 lg:-mx-8 px-4 sm:px-6 lg:px-8 pt-6 pb-8 bg-gradient-to-r from-slate-800 to-indigo-900 mb-6">
        <h2 class="font-semibold text-xl text-white leading-tight">Nuevo punto de acceso</h2>
        <p class="mt-1 text-sm text-slate-300">Nombre libre: puerta principal, vidrio, vehicular, etc.</p>
    </div>
    <div class="py-6">
                ...
                <form method="POST" action="{{ route('access.locations.store') }}">
                    ...
                    <div class="grid grid-cols-1 gap-4" x-data="geoCapture()">
                        ...
                        <div>
                            ...
                            <input type="text" name="address" value="{{ old('address') }}" class="mt-1 block w-full rounded-md bg-slate-950 border-slate-700 text-white focus:border-indigo-500 focus:ring-indigo-500">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-300">Teléfono</label>
                            <input type="text" name="phone" value="{{ old('phone') }}" class="mt-1 block w-full rounded-md bg-slate-950 border-slate-700 text-white focus:border-indigo-500 focus:ring-indigo-500">
                        </div>
                        <div>
                            <label class="inline-flex items-center">
                                <input type="checkbox" name="is_active" value="1" checked class="rounded border-slate-700 text-indigo-600 shadow-sm focus:ring-indigo-500">
